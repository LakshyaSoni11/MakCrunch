...
const Hero = forwardRef<HTMLDivElement, HeroProps>(({ onContactClick, onViewGrades }, ref) => {
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center justify-center pt-20 px-6 overflow-hidden">
      <div className="absolute inset-0 z-0">
        {/* <div className="absolute top-20 left-10 w-64 h-64 bg-emerald-200 rounded-full opacity-20 blur-3xl floating-shape"></div> */}
      ...
      <div ref={floatingImagesRef} className="absolute inset-0 z-0 pointer-events-none bg-transparent">
        {floatingImages.map((img, index) => (
          <img key={index} src={img.src} alt="Floating" className={img.className} />
        ))}
      </div>
      ...
      <div className="relative z-10 text-center max-w-6xl">
        <div className="hero-badge inline-flex items-center space-x-2 px-6 py-3 bg-white/90 backdrop-blur-sm rounded-full mb-8 shadow-xl border border-emerald-100">
          ...
          <span className="text-emerald-800 font-bold text-sm">India's #1 Raw Makhana Exporter</span>
        </div>

        <h1 className="hero-title text-5xl md:text-7xl lg:text-8xl font-black text-gray-900 mb-6 leading-tight">
          Premium Raw<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-emerald-800">Fox Nuts</span>
        </h1>

        <p className="hero-subtitle text-xl md:text-2xl text-gray-700 mb-10 max-w-3xl mx-auto font-medium">
          Direct from Bihar's finest lotus ponds. Bulk export of unprocessed Makhana for manufacturers, processors & retailers worldwide.
        </p>
        ...
        <div className="hero-cta flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <button
            ...
            className="px-10 py-5 bg-white text-emerald-700 text-lg font-bold rounded-2xl shadow-xl hover:shadow-2xl transition-all inline-flex items-center space-x-3 border-2 border-emerald-200 hover:-translate-y-1"
          >
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
          {stats.map((stat, index) => (
            <div key={index} className="about-stat bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-emerald-100">
